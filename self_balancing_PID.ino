...
float kp = 8; //--------------------------------------------------------------------Saída proporcional ao erro de inclinação do robô
float ki = 0; //-------------------------------------------------------------------Integra o erro pelo tempo para corrigir pequenos acúmulos ao longo do tempo
float kd = 0; //-------------------------------------------------------------------Deriva o erro pelo tempo para determinar sentido e intensidade da inclinação
...
void pid_control()
{
  ...
  //if (abs(error) < 0.3)//------------------------------------------------------------Cálculo da Zona Morta
  //{
    //error = 0;
  //}
  ...
  pid_I += ki * error * elapsedTime; //----------------------------------------------Cálculo da componente I
  pid_I = constrain(pid_I, -100, 100);//---------------------------------------------Anti-Windup
...
void motor_control()
{
  //Observação: Ângulos muito grandes, com magnitude acima de 50º, desligam as rodas.
  if(PID > 0)
  {
...
void loop()
{
  ...
  if(abs(Total_angle[0]) > 45)//-----------------------------------------------------Função para parar caso o ângulo fique maior que 45 graus
{
    stopMotors();
    return;
}
